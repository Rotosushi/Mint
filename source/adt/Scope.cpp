...
//   // this finds the same ubd twice as well!
//   auto result = lookupUseBeforeDefWithinThisScope(undef, q_undef);
//   if (result)
...
//     auto scope = m_prev_scope.lock();
//     auto result = scope->lookupUseBeforeDefAboveLocalScope(undef, q_undef,
//                                                            local_scope_name);
//     if (!result.empty())
//       append(results, result);
//   }
...
// [[nodiscard]] auto
// Scope::lookupUseBeforeDefAboveLocalScope(Identifier undef, Identifier
// q_undef,
...
//       auto result = scope->lookupUseBeforeDefAtParallelScope(undef, q_undef);
//       if (!result.empty())
//         append(results, result);
//     }
//   }
...
// [[nodiscard]] auto
// Scope::lookupUseBeforeDefAtParallelScope(Identifier undef,
//                                          Identifier q_undef) noexcept
//     -> std::vector<UseBeforeDefMap::Range> {
//   std::vector<UseBeforeDefMap::Range> results;
...
//     -> std::vector<UseBeforeDefMap::Range> {
//   std::vector<UseBeforeDefMap::Range> results;
//   // search for either qualified or unqualified ubds
//   auto result = lookupUseBeforeDefWithinThisScope(undef, q_undef);
//   if (result)
//     results.push_back(result.value());
...
// [[nodiscard]] auto
// Scope::lookupUseBeforeDefWithinThisScope(Identifier undef,
//                                          Identifier q_undef) noexcept
//     -> std::optional<UseBeforeDefMap::Range> {
//   auto result = m_use_before_def_map.lookup(undef, q_undef);
...
//       stage.emplace_back(usedef.names, std::move(ast), usedef.scope, false);
...
//     // #NOTE:
//     //  when we resolve the comptime value of a use before def,
...
//   // to resolve for a given binding. so we
//   // may safely remove all use-before-def
//   // bindings.
